...
class Backend(DataFusionBackend, CanCreateConnections):
    ...
    def execute(self, expr: ir.Expr, **kwargs: Any):
        super()._register_deferred_cached_tables(expr)
    ...
    def table(
        ...
        for backend in backends:
            try:
                t = backend.table(name, schema=schema, database=database)
    ...
    def list_tables(
        ...
        return [
            t
            for backend in backends
            for t in backend.list_tables(like=like, database=database)
        ]
